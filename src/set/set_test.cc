...
TEST(SetConstructors, InitializerListConstructor) {
  ...
  EXPECT_EQ((*(s21_set.find(1))).first, 1);
  EXPECT_EQ((*(s21_set.find(2))).first, 2);
  EXPECT_EQ((*(s21_set.find(3))).first, 3);
...
TEST(SetConstructors, CopyConstructor) {
  ...
  EXPECT_EQ((*(s21_setcopy.find(1))).first, 1);
  EXPECT_EQ((*(s21_setcopy.find(2))).first, 2);
  EXPECT_EQ((*(s21_setcopy.find(3))).first, 3);
...
TEST(SetConstructors, MoveConstructor) {
  ...
  EXPECT_TRUE(s21_set.empty());
  EXPECT_EQ((*(s21_setmove.find(1))).first, 1);
  EXPECT_EQ(((*(s21_setmove.find(3)))).first, 3);
...
TEST(SetIterators, Begin) {
  ...
  EXPECT_EQ((*s21_set.begin()).first, 1);
...
TEST(SetModifiers, Insert) {
  ...
  EXPECT_EQ((*(s21_set.find(4))).first, 4);
...
TEST(SetModifiers, Erase2) {
  s21::set<int> s21_set = {10, 1, 20, 5, 12};

  std::set<int> std_set = {10, 1, 20, 5, 12};
  ...
  EXPECT_EQ(s21_set.size(), std_set.size());
  EXPECT_EQ((*(s21_set.find(12))).first, 12);
  EXPECT_EQ(*(std_set.find(12)), 12);
  EXPECT_EQ((*(s21_set.find(20))).first, 20);
  EXPECT_EQ(*(std_set.find(20)), 20);
  EXPECT_FALSE(s21_set.contains(1));
}

TEST(SetModifiers, Erase3) {
  s21::set<int> s21_set = {10, 1, 20, 5, 12, 102, 3, 13};

  std::set<int> std_set = {10, 1, 20, 5, 12, 102, 3, 13};
...
TEST(SetModifiers, Erase4) {
  ...
  EXPECT_EQ(s21_set.size(), std_set.size());
  EXPECT_EQ((*(s21_set.find(102))).first, 102);
  EXPECT_EQ(*(std_set.find(102)), 102);
  EXPECT_FALSE(s21_set.contains(1));
}
...
TEST(SetModifiers, Erase5) {
  ...
  std::set<int> std_set = {10, 1, 20, 5, 12, 102, 3, 13, 1000, 344, 56, 3425};
  ...
  EXPECT_EQ(s21_set.size(), std_set.size());
  EXPECT_EQ((*(s21_set.find(3425))).first, 3425);
  EXPECT_EQ(*(std_set.find(3425)), 3425);
  EXPECT_FALSE(s21_set.contains(1));
}
...
TEST(SetModifiers, Swap) {
  ...
  EXPECT_EQ((*s21_setswap.find(3)).first, 3);
